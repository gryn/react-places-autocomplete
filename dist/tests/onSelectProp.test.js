...
describe('onSelect prop', function () {
  ...
  test('pressing Enter when one of the suggestion items is active', function () {
    ...
    input.simulate('keydown', { key: 'Enter' });
    expect(onSelectHandler).toHaveBeenCalledTimes(1);
    expect(onSelectHandler).toBeCalledWith(_testHelpers.mockSuggestions[0].description, _testHelpers.mockSuggestions[0].placeId, expect.objectContaining({ id: _testHelpers.mockSuggestions[0].id }));
  });
